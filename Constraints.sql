...
UPDATE key_table SET priamry_column = 2 WHERE primary_column = 1;
...
-- ON UPDATE참조하고 있는 테이블의 기본키가 변경될 때 동작
-- ON DELETE참조하고 있는 테이블의 기본키가 삭제될 때 동작
...
-- DEFAULT 제약조건 : 해당 컬럼의 삽입시 값이 지정되지 않으면 기본값으로 지정하는 제약
