...
-- 회원가입 
-- 최종 사용자 (아이디, 비밀번호, 비밀번호 확인, 이름, 전화번호, 인증번호)
-- 요양사 테이블에 레코드 (아이디, 암호화된 비밀변호, 이름, 전화번호, 가입경로,)
-- 필요한 추가 데이터
...
UPDATE tel_auth SET auth_number = '0123' WHERE tel_number = '012345678';
...
SELECT COUNT(*), CEIL(COUNT(*)) FROM customer;
...
-- 고객 테이블에서 이름이 포함되어 있는 레코드들을 페이지 번호에 해당하는 위치부터 10명의 고객을 내림차순 반환
...
DELETE FROM customer WHERE coustomer_number = 1;
...
-- 고객 테이블에서 특정 고객을 조회하여 반환, 담당자 이름을 반환하기 위해 영양사 테이블을 조인
